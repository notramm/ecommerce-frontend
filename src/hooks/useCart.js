...
import { getCart, updateCartItem, removeCartItem,
         applyCoupon, removeCoupon, clearCart }  from '../api/cart.api';
...
export function useServerCart() {
  ...
  const removeMutation = useMutation({
    ...
    onSuccess:  (res) => { setServerCart(res.data.data); invalidate(); toast.success('Item removed'); },
    onError:    ()    => toast.error('Failed to remove item'),
  });

  const couponMutation = useMutation({
    mutationFn: (code) => applyCoupon(code),
    onSuccess:  (res) => { setServerCart(res.data.data); invalidate(); toast.success('Coupon applied!'); },
    onError:    (e)   => toast.error(e.response?.data?.message || 'Invalid coupon'),
  });

  const removeCouponMutation = useMutation({
    mutationFn: removeCoupon,
    onSuccess:  (res) => { setServerCart(res.data.data); invalidate(); toast.success('Coupon removed'); },
  });

  const clearMutation = useMutation({
    mutationFn: clearCart,
    onSuccess:  () => { setServerCart(null); invalidate(); toast.success('Cart cleared'); },
  });
