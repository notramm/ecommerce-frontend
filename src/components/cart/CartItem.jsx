...
export default function CartItem({ item, onUpdate, onRemove, isUpdating }) {
  ...
  const itemId = item._id; // ← _id exist karta hai confirmed
      ...
      {!removing && (
        ...
        >
          <div className="flex gap-4 sm:gap-5 py-5 sm:py-6 border-b border-white/[0.06]">
            {/* Image */}
            <div className="shrink-0 w-20 h-24 sm:w-24 sm:h-28 bg-[#111] border border-white/[0.06] overflow-hidden">
              {image ? (
                <img
                  src={image}
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <span className="text-stone/20 text-xs">No img</span>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between">
              <div>
                <p className="text-cream text-sm sm:text-base font-medium line-clamp-2 mb-1.5">
                  {item.nameSnapshot}
                </p>
                ...
                {item.priceChanged && (
                  <p className="text-[10px] text-yellow-500 font-mono mb-2">
                    Price updated to ₹{item.variant?.price}
                  </p>
                )}
              </div>
              ...
              <div className="flex items-end justify-between gap-4 flex-wrap">
                ...
                <div className="flex items-center border border-white/[0.1]">
                  ...
                  <button
                    onClick={() => onUpdate(itemId, item.quantity + 1)}
                    disabled={
                      isUpdating || item.quantity >= (item.variant?.stock || 10)
                    }
                    className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-stone hover:text-cream transition-colors disabled:opacity-30"
                  >
                    <Plus size={12} />
