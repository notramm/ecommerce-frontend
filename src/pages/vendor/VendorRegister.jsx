...
export default function VendorRegister() {
  ...
  const { updateUser, setAccessToken, logout } = useAuthStore();
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: async () => {
      toast.success("Vendor account created! Refreshing session...");

      try {
        // Get fresh token with role=vendor
        ...
        if (newToken) {
          ...
          updateUser({ role: decoded.role || "vendor" });
          ...
          if (stored) {
            try {
              ...
              if (parsed.state.user)
                parsed.state.user.role = decoded.role || "vendor";
              localStorage.setItem("luxe-auth", JSON.stringify(parsed));
            } catch {}
          }
          toast.success("Session updated! Welcome to Vendor Portal.");
          navigate("/vendor/kyc");
        }
      } catch {
        toast.info("Please login again to activate your vendor access.");
        logout();
  ...
  const Field = ({ name, label, placeholder, required, hint }) => (
    <div>
      ...
      {hint && !errors[name] && (
        <p className="text-[10px] text-stone/30 mt-1">{hint}</p>
      )}
    </div>
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-20 items-start">
          ...
          >
            ...
            <p className="text-stone text-sm leading-relaxed mb-8 max-w-sm">
              Join thousands of vendors already selling on LUXE.
            </p>
              ...
              {STEPS_INFO.map((s, i) => (
                <div key={s.n} className="flex items-center gap-3">
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    <p className="text-[9px] text-stone/40 uppercase tracking-wider text-center">
                      {s.label}
                    </p>
                  </div>
              ...
              <div className="bg-[#0d0d0d] border border-white/[0.07] p-5 sm:p-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-[#0a0a0a] border border-white/[0.07] focus:border-gold/40 text-cream placeholder:text-stone/25 px-4 py-3 text-sm outline-none resize-none"
                  />
                </div>
