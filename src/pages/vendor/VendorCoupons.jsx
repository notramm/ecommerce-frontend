...
import { Plus, Tag, Trash2, ToggleLeft, ToggleRight, Loader2, X } from 'lucide-react';
import PageWrapper   from '../../components/layout/PageWrapper';
import DashboardShell from '../../components/layout/DashboardShell';
import { Skeleton }  from '../../components/ui/Skeleton';
import { getVendorCoupons, createVendorCoupon, updateVendorCoupon, deleteVendorCoupon } from '../../api/vendor.api';
...
function CreateCouponDrawer({ open, onClose, queryClient }) {
  const { register, handleSubmit, watch, reset, formState: { errors } } = useForm({
    ...
    defaultValues: { discountType: 'flat', minOrderValue: 0, maxUsesPerUser: 1 },
  });
  const discountType = watch('discountType');

  const mutation = useMutation({
    mutationFn: (d) => createVendorCoupon({
      ...d,
        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
              <Field name="code"        label="Coupon Code"    placeholder="SAVE20"    required />
              <Field name="description" label="Description"    placeholder="20% off on all items" />

              <div>
                <label className="eyebrow text-stone/40 text-[10px] block mb-2">Discount Type</label>
                <div className="flex gap-2">
                  {['flat', 'percent'].map((t) => (
                    <label key={t} className="flex-1 flex items-center justify-center gap-2 cursor-pointer">
                      <input {...register('discountType')} type="radio" value={t} className="hidden peer" />
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field
                  name="discountValue"
                  label={discountType === 'flat' ? 'Amount (₹)' : 'Percentage (%)'}
                  placeholder={discountType === 'flat' ? '100' : '20'}
                  type="number"
                  required
                />
                {discountType === 'percent' && (
                  <Field name="maxDiscount" label="Max Discount (₹)" placeholder="500" type="number" />
                )}
              </div>

              <div className="grid grid-cols-2 gap-3">
                <Field name="minOrderValue"  label="Min Order (₹)" placeholder="499" type="number" />
                <Field name="maxUsesPerUser" label="Per User Limit" placeholder="1"   type="number" />
              </div>

              <Field name="maxUses"   label="Total Max Uses" placeholder="Leave empty for unlimited" type="number" />
              <Field name="expiresAt" label="Expiry Date"    placeholder=""           type="datetime-local" required />
            </div>
...
export default function VendorCoupons() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn:  async () => { const { data } = await getVendorCoupons(); return data.data.coupons; },
    staleTime: 2 * 60 * 1000,
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess:  () => { toast.success('Coupon deleted'); queryClient.invalidateQueries({ queryKey: ['vendor-coupons'] }); },
    onError:    (e) => toast.error(e.response?.data?.message || 'Cannot delete used coupon'),
  });
      ...
      <DashboardShell title="Coupons" subtitle="Vendor">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="font-display text-xl text-cream mb-2">No coupons yet</p>
            <p className="text-stone text-sm mb-6">Create discount coupons to attract more customers</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="font-mono text-sm text-gold font-medium">{c.code}</span>
                        <span className={cn(
