...
import { Plus, Package, Edit2, Eye, TrendingDown, Loader2,
         Upload, X, AlertCircle } from 'lucide-react';
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07] shrink-0">
              ...
              <button onClick={onClose} className="p-1.5 text-stone hover:text-cream transition-colors"><X size={16} /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
              ...
              <div>
                ...
                {errors.categoryId && <p className="text-[10px] text-vermillion/80 mt-1">{errors.categoryId.message}</p>}
              </div>
              ...
              {/* Images */}
              <div>
                <label className="eyebrow text-stone/40 text-[10px] block mb-2">Product Images (max 5)</label>
...
export default function VendorProducts() {
  ...
  const { data: catData } = useQuery({
    queryKey:  ['categories'],
    queryFn:   async () => { const { data } = await getCategoryTree(); return data.data; },
    staleTime: 10 * 60 * 1000,
  });

  const products   = data?.data?.products || [];
  const categories = catData?.filter((c) => !c.parent) || [];
      ...
      <DashboardShell title="My Products" subtitle="Vendor">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <button onClick={() => setDrawerOpen(true)} className="btn-primary text-sm flex items-center gap-2 mx-auto">
