...
import { Upload, CheckCircle2, Clock, XCircle, Loader2, Building, CreditCard } from 'lucide-react';
import PageWrapper      from '../../components/layout/PageWrapper';
import DashboardShell   from '../../components/layout/DashboardShell';
import { getVendorProfile, submitKYC, updateBankDetails } from '../../api/vendor.api';
...
function KYCStatusBadge({ status }) {
  const map = {
    approved:      { label: 'Approved',       icon: CheckCircle2, className: 'text-emerald-400 border-emerald-400/20 bg-emerald-400/5' },
    pending:       { label: 'Under Review',   icon: Clock,        className: 'text-yellow-500 border-yellow-500/20 bg-yellow-500/5' },
    rejected:      { label: 'Rejected',       icon: XCircle,      className: 'text-vermillion border-vermillion/20 bg-vermillion/5' },
    not_submitted: { label: 'Not Submitted',  icon: Upload,       className: 'text-stone border-white/[0.1] bg-white/[0.02]' },
  };
  const { label, icon: Icon, className } = map[status] || map.not_submitted;
  return (
    <span className={cn('inline-flex items-center gap-1.5 text-xs font-mono border px-3 py-1.5', className)}>
      <Icon size={12} />
      {label}
    </span>
  );
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          {doc.required && <span className="text-[10px] text-vermillion/60 font-mono">Required</span>}
        </div>
        {existing && (
          <span className={cn(
            'text-[10px] font-mono border px-2 py-0.5',
            existing.status === 'approved' ? 'text-emerald-400 border-emerald-400/20' :
            existing.status === 'pending'  ? 'text-yellow-500 border-yellow-500/20'   :
            existing.status === 'rejected' ? 'text-vermillion border-vermillion/20'   :
      ...
      {preview ? (
        <div className="relative w-full h-24 mb-3 overflow-hidden bg-[#111] border border-white/[0.06]">
          ...
          <button
            onClick={() => { setPreview(null); onChange(doc.key, null); if (inputRef.current) inputRef.current.value = ''; }}
        ...
        <div className="w-full h-16 mb-3 overflow-hidden bg-[#111] border border-white/[0.06] flex items-center justify-center">
          <img src={existing.url} alt={doc.label} className="h-full object-contain" />
        </div>
      ) : null}
      ...
      <input
        ...
        onChange={handleFile}
      />
      <p className="text-[10px] text-stone/25 mt-1.5 text-center">JPG, PNG or PDF • Max 5MB</p>
    </div>
  );
}
...
export default function VendorKYC() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn:  async () => { const { data } = await getVendorProfile(); return data.data.vendor; },
    staleTime: 2 * 60 * 1000,
  });

  const vendor = data;

  const { register: bankReg, handleSubmit: bankSubmit, formState: { errors: bankErrors } } = useForm({
  ...
  const kycMutation = useMutation({
    ...
    onError: (e) => toast.error(e.response?.data?.message || 'KYC submission failed'),
  });

  const bankMutation = useMutation({
    mutationFn: updateBankDetails,
    onSuccess:  () => { toast.success('Bank details saved'); queryClient.invalidateQueries({ queryKey: ['vendor-profile'] }); },
      ...
      <DashboardShell title="KYC Verification" subtitle="Vendor">
        ...
        {!isLoading && vendor && (
          <div className="flex items-start sm:items-center justify-between gap-4 bg-[#0d0d0d] border border-white/[0.07] p-4 sm:p-5 mb-6 flex-col sm:flex-row">
            <div>
              <p className="text-cream text-sm font-medium mb-1">Verification Status</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-[#0d0d0d] border border-white/[0.07] p-5 sm:p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <p className="eyebrow text-stone/40 text-[10px]">Identity Documents</p>
            </div>
            ...
            <motion.button
              ...
              disabled={!hasFiles || kycMutation.isPending || vendor?.kycStatus === 'pending'}
              whileTap={{ scale: 0.97 }}
              className="w-full btn-primary py-3.5 flex items-center justify-center gap-2 disabled:opacity-40"
            >
              {kycMutation.isPending && <Loader2 size={14} className="animate-spin" />}
              {vendor?.kycStatus === 'pending'
                ? 'Pending Review'
                : vendor?.kycStatus === 'approved'
                  ? 'Re-submit Documents'
          ...
          <div className="bg-[#0d0d0d] border border-white/[0.07] p-5 sm:p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <p className="eyebrow text-stone/40 text-[10px]">Bank Details (for payouts)</p>
            </div>

            <form onSubmit={bankSubmit((d) => bankMutation.mutate(d))} className="space-y-4">
              {[
                { name: 'accountHolderName', label: 'Account Holder Name',  placeholder: 'Full name as on bank account' },
                { name: 'accountNumber',     label: 'Account Number',        placeholder: '1234567890123' },
                { name: 'ifscCode',          label: 'IFSC Code',             placeholder: 'SBIN0001234' },
                { name: 'bankName',          label: 'Bank Name',             placeholder: 'State Bank of India' },
                { name: 'branchName',        label: 'Branch Name',           placeholder: 'Fort Branch (optional)' },
                ...
                <div key={name}>
                  ...
                  <input
                    ...
                    className={cn(
                      ...
                      bankErrors[name] ? 'border-vermillion/40' : 'border-white/[0.07]'
                    )}
                  />
                  {bankErrors[name] && (
                    <p className="text-[10px] text-vermillion/80 mt-1">{bankErrors[name].message}</p>
                  )}
                </div>
              ))}
              ...
              >
                {bankMutation.isPending && <Loader2 size={14} className="animate-spin" />}
                Save Bank Details
              </motion.button>
            </form>
          </div>
