...
import { TrendingUp, Package, ShoppingBag, DollarSign,
         AlertTriangle, ArrowUpRight } from 'lucide-react';
...
export default function VendorDashboard() {
  ...
  const { data: dashData, isLoading } = useQuery({
    ...
    queryFn:   async () => { const { data } = await getVendorDashboard(); return data.data; },
    staleTime: 2 * 60 * 1000,
  });

  const { data: analyticsData } = useQuery({
    queryKey:  ['vendor-analytics', period],
    queryFn:   async () => { const { data } = await getSalesAnalytics(period); return data.data; },
    staleTime: 5 * 60 * 1000,
  });
  ...
  const stats = [
    { label: 'Today\'s Revenue', value: formatPrice(summary.today?.revenue || 0),    sub: `${summary.today?.orderCount || 0} orders`, icon: DollarSign, accent: true },
    { label: 'This Month',       value: formatPrice(summary.thisMonth?.revenue || 0), sub: `${summary.thisMonth?.orderCount || 0} orders`, icon: TrendingUp },
    { label: 'Pending Payout',   value: formatPrice(vendor.pendingPayout || 0),       sub: 'Available to withdraw', icon: DollarSign },
    { label: 'Total Products',   value: (vendor.totalProducts || 0).toString(),        sub: 'Active listings', icon: Package },
  ];
      ...
      <DashboardShell title="Dashboard" subtitle="Vendor Overview">
        ...
        {vendor.kycStatus && vendor.kycStatus !== 'approved' && (
          ...
          >
            ...
            <Link to="/vendor/kyc" className="btn-outline text-xs whitespace-nowrap border-yellow-500/30 text-yellow-500 hover:border-yellow-500/60 shrink-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-[#0d0d0d] border border-white/[0.07] p-5 sm:p-6">
            <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
              <div>
                <p className="eyebrow text-stone/40 text-[10px] mb-1">Revenue Trend</p>
            ...
            {chartData.length > 0
              ...
              : <div className="h-20 flex items-center justify-center text-stone/30 text-xs">No data for this period</div>
            }
          </div>
          ...
          <div className="bg-[#0d0d0d] border border-white/[0.07] p-5 sm:p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <Link to="/vendor/products" className="text-[10px] text-gold/60 hover:text-gold transition-colors">
                View All →
              </Link>
            </div>
            {topProducts.length === 0 ? (
              <div className="text-center py-6">
                ...
                <Link to="/vendor/products" className="text-gold text-xs mt-1 block hover:underline">
                ...
                {topProducts.map((p, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-stone/40 text-[10px] font-mono">{p.units} units</p>
                    </div>
                    <p className="text-cream text-xs font-mono shrink-0">{formatPrice(p.revenue)}</p>
                  </motion.div>
                ))}
              </div>
            )}
          ...
          <div className="bg-[#0d0d0d] border border-white/[0.07] p-5 sm:p-6">
            <p className="eyebrow text-stone/40 text-[10px] mb-5">Order Breakdown</p>
            {Object.entries(orderStats).length === 0 ? (
              <p className="text-stone/40 text-xs text-center py-6">No orders yet</p>
            ) : (
              <div className="space-y-3">
                {Object.entries(orderStats).map(([status, { count, revenue }]) => (
                  <div key={status} className="flex items-center gap-3">
                    <span className="text-xs text-stone capitalize flex-1">{status.replace(/_/g, ' ')}</span>
                    <span className="text-xs font-mono text-stone/50">{count}</span>
                    <span className="text-xs font-mono text-cream">{formatPrice(revenue)}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Quick actions */}
          <div className="bg-[#0d0d0d] border border-white/[0.07] p-5 sm:p-6">
            <p className="eyebrow text-stone/40 text-[10px] mb-5">Quick Actions</p>
            <div className="space-y-2">
              {[
                { label: 'Add New Product',  href: '/vendor/products', icon: Package },
                { label: 'View Orders',      href: '/vendor/orders',   icon: ShoppingBag },
                { label: 'Request Payout',   href: '/vendor/payouts',  icon: DollarSign },
                { label: 'Create Coupon',    href: '/vendor/coupons',  icon: TrendingUp },
                ...
                >
                  <Icon size={14} className="text-stone/50 group-hover:text-gold transition-colors" />
                  <span className="text-sm text-stone group-hover:text-cream transition-colors">{label}</span>
                  <ArrowUpRight size={12} className="ml-auto text-stone/20 group-hover:text-gold transition-colors" />
                </Link>
              ))}
            </div>
          </div>
