      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-0.5">
            ...
            {user.isBanned && (
              <span className="text-[10px] font-mono border border-vermillion/30 bg-vermillion/10 text-vermillion px-2 py-0.5">
                Banned
              </span>
            )}
          </div>
      ...
      {expanded && (
        ...
        >
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div>
              <p className="text-stone/30 mb-0.5">Wallet</p>
              <p className="text-cream font-mono">₹{user.walletBalance || 0}</p>
            </div>
            <div>
              <p className="text-stone/30 mb-0.5">Providers</p>
              <p className="text-cream">{user.authProviders?.length || 0}</p>
            </div>
            <div>
              <p className="text-stone/30 mb-0.5">Phone</p>
              <p className="text-cream font-mono">{user.phone || "—"}</p>
            </div>
            <div>
              <p className="text-stone/30 mb-0.5">Last Login</p>
          ...
          {user.isBanned ? (
            <div className="flex items-center gap-3">
              <p className="text-xs text-vermillion/60 flex-1">
            ...
            <>
              {showBanForm ? (
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (banReason.trim()) onBan(user._id, banReason);
                    }}
                  ...
                  <button
                    ...
                    className="px-3 py-2 border border-white/[0.08] text-stone text-xs hover:text-cream"
                  >
                    Cancel
                  </button>
                </div>
  ...
  return (
    <PageWrapper>
      <DashboardShell title="User Management" subtitle="Admin">
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          ...
          >
            ...
            {["customer", "vendor", "agent", "admin"].map((r) => (
              <option key={r} value={r} className="bg-[#0f0f0f] capitalize">
                {r}
              </option>
            ))}
          </select>
          ...
          >
            ...
            <option value="false" className="bg-[#0f0f0f]">
              Active
            </option>
            <option value="true" className="bg-[#0f0f0f]">
              Banned
            </option>
          </select>
        </div>
        ...
        {total > 0 && (
          <p className="text-stone/40 text-xs font-mono mb-4">
            {total} users found
          </p>
        )}
        ...
        {/* Error state */}
        {error && (
          <div className="bg-vermillion/5 border border-vermillion/20 p-4 mb-4 text-xs text-vermillion/80">
