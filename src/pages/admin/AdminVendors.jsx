...
import { getPendingKYC, reviewKYC, processAdminPayout } from '../../api/admin.api';
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={cn(
            ...
            vendor.kycStatus === 'approved' ? 'text-emerald-400 border-emerald-400/20 bg-emerald-400/5' :
            vendor.kycStatus === 'pending'  ? 'text-yellow-500 border-yellow-500/20 bg-yellow-500/5' :
            vendor.kycStatus === 'rejected' ? 'text-vermillion border-vermillion/20 bg-vermillion/5' :
      ...
      {expanded && (
        ...
        >
          ...
          {vendor.documents?.length > 0 && (
            <div>
              <p className="eyebrow text-stone/30 text-[10px] mb-2">Documents</p>
                ...
                {vendor.documents.map((doc) => (
                  <a
                    ...
                    className={cn(
                      ...
                      doc.status === 'approved' ? 'text-emerald-400 border-emerald-400/20' :
                      doc.status === 'rejected' ? 'text-vermillion border-vermillion/20' :
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div><p className="text-stone/30 mb-0.5">PAN</p><p className="text-cream font-mono">{vendor.panNumber || '—'}</p></div>
            <div><p className="text-stone/30 mb-0.5">GST</p><p className="text-cream font-mono">{vendor.gstNumber || '—'}</p></div>
            <div><p className="text-stone/30 mb-0.5">Submitted</p><p className="text-cream">{vendor.kycSubmittedAt ? formatDate(vendor.kycSubmittedAt) : '—'}</p></div>
            <div><p className="text-stone/30 mb-0.5">Total Earnings</p><p className="text-cream font-mono">₹{vendor.totalEarnings || 0}</p></div>
          </div>
            ...
            <div className="space-y-2">
              {showReject ? (
                <div className="flex gap-2">
                  ...
                  />
                  <button
                    onClick={() => onReview(vendor._id, { action: 'reject', reason: rejectMsg })}
                  ...
                  <button onClick={() => setShowReject(false)} className="px-3 py-2 border border-white/[0.08] text-stone text-xs">Cancel</button>
                </div>
              ) : (
                <div className="flex gap-2">
                  <button
...
export default function AdminVendors() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn:   () => getPendingKYC({ status: kycStatus, limit: 30 }),
    staleTime: 2 * 60 * 1000,
  });

  const reviewMutation = useMutation({
    mutationFn: ({ vendorId, data }) => reviewKYC(vendorId, data),
    onSuccess:  () => { toast.success('KYC reviewed'); queryClient.invalidateQueries({ queryKey: ['admin-kyc'] }); },
          ...
          <div className="space-y-2">
            {vendors.map((v) => (
              <VendorKYCRow
                key={v._id}
                vendor={v}
                onReview={(vendorId, data) => reviewMutation.mutate({ vendorId, data })}
                onPayout={() => {}}
              />
            ))}
          </div>
