...
import { Check, MapPin, CreditCard, ShoppingBag, ChevronRight, Loader2 } from 'lucide-react';
        ...
        return (
          <div key={step.id} className="flex items-center">
            <div className="flex flex-col items-center gap-1.5">
              <motion.div
                animate={{
                  borderColor: done || active ? '#c9a96e' : 'rgba(255,255,255,0.1)',
                  backgroundColor: done ? '#c9a96e' : active ? 'rgba(201,169,110,0.1)' : 'transparent',
                }}
              ...
              >
                {done ? (
                  ...
                  <Icon size={14} className={active ? 'text-gold' : 'text-stone/30'} />
                )}
              </motion.div>
              <span className={cn(
                'text-[10px] font-mono uppercase tracking-wider hidden sm:block',
                active ? 'text-gold' : done ? 'text-stone/50' : 'text-stone/25'
              )}>
                {step.label}
              </span>
            </div>
            ...
            {i < STEPS.length - 1 && (
              <motion.div
                ...
                animate={{ backgroundColor: done ? '#c9a96e' : 'rgba(255,255,255,0.08)' }}
...
export default function CheckoutPage() {
    ...
    try {
      ...
      if (order.isCOD || order.status === 'confirmed') {
        navigate(`/order/success/${order.orderId}`, {
          state: { orderCode: order.orderCode || order.orderId, paymentMethod },
        });
        return;
      }
      ...
      if (!loaded) { toast.error('Payment service unavailable. Please try again.'); setPlacing(false); return; }
      ...
      navigate(`/order/success/${verifyRes.data.data.orderId}`, {
        state: {
          orderCode:   verifyRes.data.data.orderCode || order.orderCode,
  ...
  }, [selectedAddr, paymentMethod, walletAmount, note, user, effectiveTotal, navigate]);
    ...
    <PageWrapper noFooter>
      <div className="max-w-[1200px] mx-auto px-5 sm:px-8 lg:px-12 pt-6 sm:pt-8 pb-20">
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          <div>
            <p className="eyebrow text-gold/50 mb-1.5 text-[10px] sm:text-xs">Checkout</p>
            <h1 className="font-display text-2xl sm:text-3xl text-cream">Complete Order</h1>
          </div>
          <Link to="/cart" className="flex items-center gap-2 text-xs text-stone hover:text-cream transition-colors">
            ← Back to cart
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] xl:grid-cols-[1fr_420px] gap-8 xl:gap-12 items-start">
          ...
          <div className="min-h-[400px]">
            <AnimatePresence mode="wait">
              ...
              {/* Step 1 — Address */}
              {step === 1 && (
                <StepSection key="step1" title="Delivery Address" subtitle="Step 1 of 3">
                  <AddressSelector
              ...
              {/* Step 2 — Payment */}
              {step === 2 && (
                <StepSection key="step2" title="Payment Method" subtitle="Step 2 of 3">
                  <PaymentSelector
              ...
              {/* Step 3 — Review */}
              {step === 3 && (
                <StepSection key="step3" title="Review & Place Order" subtitle="Step 3 of 3">
                  {/* Address review */}
                  {selectedAddrObj && (
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="eyebrow text-gold/50 text-[10px]">Delivering to</p>
                          </div>
                          <p className="text-cream text-sm font-medium mb-0.5">{selectedAddrObj.fullName}</p>
                          <p className="text-stone text-xs">
                            {selectedAddrObj.line1}
                            {selectedAddrObj.line2 ? `, ${selectedAddrObj.line2}` : ''},
                            {' '}{selectedAddrObj.city}, {selectedAddrObj.state} {selectedAddrObj.pincode}
                          </p>
                          <p className="text-stone/50 text-xs font-mono mt-0.5">{selectedAddrObj.phone}</p>
                        </div>
                        <button
                          onClick={() => setStep(1)}
                  ...
                  <div className="bg-[#0f0f0f] border border-white/[0.07] p-4 sm:p-5 mb-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="eyebrow text-gold/50 text-[10px] mb-1">Payment</p>
                    ...
                    >
                      {placing ? (
                        <>
                          ...
                          {paymentMethod === 'razorpay' ? 'Preparing Payment...' : 'Placing Order...'}
                        </>
                      ) : (
                        <>
                          <ShoppingBag size={16} />
                  ...
                  <p className="text-[10px] text-stone/30 text-center mt-4">
                    ...
                    <Link to="/terms" className="underline hover:text-stone/60 transition-colors">Terms & Conditions</Link>
                  </p>
                </StepSection>
              )}
